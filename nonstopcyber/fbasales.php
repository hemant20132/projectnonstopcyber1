                ...
                <div class="row">
                    <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                        <div class="page-header">
                            <h2 class="pageheader-title">FBA Sales</h2>
                            <p class="pageheader-text">FBA Sales List </p>
												...
												else
												{
													...
													for ($j=0;$j<sizeof($platforms);$j++)
													{
														...
														<?php
															...
															{
																if ($platforms[$j]['platformname']==$platforms[$i]['platformname'] and $platform==$platforms[$i]['code'])
																{	
															?>
								   ...
								   <table id="salestable" class="table table-striped table-bordered second" style="width:100%">
                                        ...
                                        <?php
											...
											if (isset($_REQUEST['filter']))
											{
												if (strtotime($_REQUEST['fromdate']))
												{
													$fromdate=date("Y-m-d H:",strtotime($_REQUEST['fromdate']));
												}
												else
												{
													$fromdate="";		
												...
												else
												{
													...
													if ($_SESSION['role']=='Admin' OR $_SESSION['role']=='Superuser')
													{
														...
														{
															$query = "SELECT * FROM fbasale where platform like  '%FBA%'  and platform='".$platform."'";
														}
														if ($fromdate!="" and $todate=="" and $platform != "")
														{
															$query = "SELECT * FROM fbasale where platform like  '%FBA%'  and tdate >= '".$fromdate."' and platform='".$platform."'";
														}
														if ($fromdate=="" and $todate!="" and $platform != "")
														{
															$query = "SELECT * FROM fbasale where platform like  '%FBA%'  and tdate <= '".$todate."' and platform='".$platform."'";
														}
														if ($fromdate!="" and $todate!="" and $platform != "")
														{
															$query = "SELECT * FROM fbasale where platform like  '%FBA%'   and tdate>= '".$fromdate."' and tdate <= '".$todate."' and platform='".$platform."'";
														}
													}
											...
											else
											{
												if (isset($_REQUEST['searchorder']) and ($_SESSION['role']=='Admin' or $_SESSION['role']=='Superuser'))
													{
														$query = "SELECT * FROM fbasale where  platform like  '%FBA%'  and  orderno='".$_REQUEST['searchorder']."'";
													}
												...
												if (!isset($_REQUEST['searchorder']) and ($_SESSION['role']=='Admin' or $_SESSION['role']=='Superuser'))
													{
														$query = "SELECT * FROM fbasale where platform like '%FBA%' order by id";
													}
												if (!isset($_REQUEST['searchorder']) and ($_SESSION['role']=='User'))
													{	
														$query = "SELECT * FROM fbasale  where platform like '%FBA%' order by id";
											...
											$i = 0;
												...
												if (mysqli_num_rows($result) > 0) 
												{
													...
													?>
<tr><td><?php echo $i; ?></td><td><div class="m-r-10"><img src="fbasaleimages/<?php echo $row['sale_image'];?>" alt="saleimage" class="rounded" height="60" width="45"></div></td><td><?php echo $row['orderno'];?></td><td><?php echo date('d-m-Y',strtotime($row['tdate']));?></td><td><?php echo $platformname;?></td><td><?php echo $row['product_value'];?></td><td><?php echo $row['commissioncharges'];?></td><td><?php echo $row['shippingcharges'];?></td><td><?php echo $row['totalamount'];?></td><td><?php echo $row['addtinfo']; ?><td><a href="fba-sales-details-single.php?saleid=<?php echo $row['id'];?>" class='btn btn-rounded btn-primary btn-sm float-right'>View Details</a></td><?php if ($_SESSION['role']=='Superuser' or $_SESSION['role']=='Admin' or ($_SESSION['role']=='User' AND $_SESSION['username']==$row['username'])){ ?><td><a href="edit-fba-sale.php?saleid=<?php echo $row['id'];?>" class='btn btn-rounded btn-success btn-sm float-right'>Edit Details</a></td><?php } ?><?php if ($_SESSION['role']=='Superuser' OR $_SESSION['role']=='Admin' OR ($_SESSION['role'] == 'User' AND $_SESSION['username'] == $row['username'])){ ?><td><a href="" data-id="<?php echo $row['id']; ?>" class="btn btn-rounded btn-danger btn-sm float-right deletesale">Delete Details</a></td><?php } ?></tr>	
													<?php
	...
	<script> 
		...
		{ 
			...
			responsive: true,
				...
				buttons: 
				[
				{text:'Add FBA Sale',className:'btn btn-primary addsale',
						action: function ( e, dt, node, config ) 
						{ location.href='fbasale.php'
						}
				},
				'copy',
				{
					...
					{text:'excel',className:'btn btn-primary excelreport',
						action: function ( e, dt, node, config ) 
						{ location.href='fbasaleexcelreport.php'
						}
				},
				{
                    extend: 'pdf',
				...
				{text:'FBA Daily Sales',className:'btn btn-primary dailysale',
						action: function ( e, dt, node, config ) 
						{ location.href='fba-daily-sale.php'
						}
				}
				]
			});
