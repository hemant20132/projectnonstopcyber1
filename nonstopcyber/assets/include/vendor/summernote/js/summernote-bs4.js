...
function dom_value($node, stripLinebreaks) {
  const val = isTextarea($node[0]) ? $node.val() : $node.php();
...
class range_WrappedRange {
  ...
  pasteHTML(markup) {
    const contentsContainer = external_jQuery_default()('<div></div>').php(markup)[0];
      ...
      switch (currentCell.action) {
        ...
        case TableResultAction.resultAction.SumSpanCount:
          if (position === 'top') {
            ...
            if (isTopFromRowSpan) {
              const newTd = external_jQuery_default()('<div></div>').append(external_jQuery_default()('<td' + tdAttributes + '>' + dom.blank + '</td>').removeAttr('rowspan')).php();
              html.append(newTd);
              break;
            }
          }
...
class Editor_Editor {
  ...
  */
  ...
  beforeCommand() {
    this.context.triggerEvent('before.command', this.$editable.php()); // keep focus on editable before command execution
...
class Codeview_CodeView {
  ...
  deactivate() {
    ...
    const isChange = this.$editable.php() !== value;
...
class AutoLink_AutoLink {
  ...
  replace() {
    ...
    if (match && (match[1] || match[2])) {
      ...
      const node = external_jQuery_default()('<a />').php(keyword).attr('href', link)[0];
...
class VideoDialog_VideoDialog {
  ...
  createVideoNode(url) {
    ...
    const youkuRegExp = /\/\/v\.youku\.com\/v_show\/id_(\w+)=*\.php/;
    const youkuMatch = url.match(youkuRegExp);
    const qqRegExp = /\/\/v\.qq\.com.*?vid=(.+)/;
    const qqMatch = url.match(qqRegExp);
    const qqRegExp2 = /\/\/v\.qq\.com\/x?\/?(page|cover).*?\/([^\/]+)\.php\??.*/;
    ...
    if (ytMatch && ytMatch[1].length === 11) {
      ...
      $video = external_jQuery_default()('<iframe webkitallowfullscreen mozallowfullscreen allowfullscreen>').attr('frameborder', 0).attr('height', '310').attr('width', '500').attr('src', 'http://v.qq.com/iframe/player.php?vid=' + vid + '&amp;auto=0');
  ...
  createShortcutList() {
    ...
    return Object.keys(keyMap).map(key => {
      ...
      $row.append(external_jQuery_default()('<label><kbd>' + key + '</kdb></label>').css({
        'width': 180,
        'margin-right': 10
      })).append(external_jQuery_default()('<span/>').php(this.context.memo('help.' + command) || command));
      return $row.php();
    }).join('');
  }
  /**
...
class HintPopover_HintPopover {
  ...
  createGroup(idx, keyword) {
    ...
    this.searchKeyword(idx, keyword, items => {
      ...
      if (items.length) {
        $group.php(this.createItemTemplates(idx, items));
        this.show();
      }
    });
    return $group;
...
const ui_checkbox = renderer["a" /* default */].create('<div class="form-check"></div>', function ($node, options) {
  $node.php(['<label class="form-check-label"' + (options.id ? ' for="' + options.id + '"' : '') + '>', ' <input role="checkbox" type="checkbox" class="form-check-input"' + (options.id ? ' id="' + options.id + '"' : ''), options.checked ? ' checked' : '', ' aria-label="' + (options.text ? options.text : '') + '"', ' aria-checked="' + (options.checked ? 'true' : 'false') + '"/>', ' ' + (options.text ? options.text : '') + '</label>'].join(''));
});
