		...
		$brandlist[]=$rowbrand;
            ...
            if (move_uploaded_file($_FILES["importfile"]["tmp_name"], $target_file)) 
            {
                ...
                if ($row=SimpleXLSX::parse($inputFileName)) 
                {
					...
					$productsecondname=str_replace("'","\'",$rows[$i][1]);
					$productsecondname=str_replace('"','\"',$productsecondname);
					$productsecondname=str_replace('"','\"',$productsecondname);
					$productsecondname=str_replace("`","\`",$productsecondname);
					$productsecondname=str_replace("•","&#8226;",$productsecondname);
					$productsecondname=str_replace("®","&circledR;",$productsecondname);
					
					$productbarcode=$rows[$i][2];
					
					$productname=str_replace("'","\'",$rows[$i][3]);
					...
					for ($k=0;$k<sizeof($brandlist);$k++)
					{
						if (trim($productbrand)==trim($brandlist[$k]['brand']))
						{
								...
								array_push($productimages1,$newFile);
								$count++;
								}
					$productimages=implode(",",$productimages1);
					$purchasedate=NULL;
					...
					short_description, long_description, shelf_number, purchase_date) 
					values('".$productname."',".$productbarcode.",'".$categorycode."','".$subcategorycode."','".$brandcode."','".$productnsinumber."','".$productimages
					."','".$productsize."','".$productsecondname."','".$productshort."','".$productlong."','".$productshelf."','".($purchasedate==NULL?"NULL":"'$purchasedate'")."')";
					echo $productinsert;
