...
header("Content-Disposition: attachment; filename=salesreport.xls");  //File name extension was wrong
header("Expires: 0");
...
$sales="select * from sale where DATE(tdate)='".$today1."' order by id";
...
$salepro="select * from sale_products order by id";
...
$salereport="<center><p style='font-size:15pt;'>NonStopCyber -- Daily Sale Report- ".$today."</p><br/></center>";
...
for ($i=0;$i<sizeof($salearr);$i++)
{
			...
			for ($j=0;$j<sizeof($saleprarr);$j++)
			{
				...
				{
					$salereport.="<tr><td colspan='3'></td><td>".$platformname."</td><td>".$saleprarr[$j]['barcode']."</td><td>".$saleprarr[$j]['nsinumber']."</td><td>".$saleprarr[$j]['productname']."</td><td>".$saleprarr[$j]['quantity']."</td><td>".$saleprarr[$j]['rate']."</td></tr>";
