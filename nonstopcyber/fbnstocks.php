                ...
                <div class="row">
                    <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                        <div class="page-header">
                            <h2 class="pageheader-title">FBN Stocks</h2>
                            ...
                            <div class="page-breadcrumb">
                                <nav aria-label="breadcrumb">
                                    <ol class="breadcrumb">
                                        ...
                                        <li class="breadcrumb-item active" aria-current="page">FBN Stocks</li>
                                    </ol>
                                </nav>
                            </div>
                        </div>
								...
								-->	
									...
									<div class="row" style="margin-top:20px;">
										<?php if ($_SESSION['role']=='Superuser' or $_SESSION['role']=='Admin') 
												{
										?>
											<input  id="stocksendfbn" name="stocksendfbn" type="button" style="margin-left:15px;" class="btn btn-default" value="Stock Send FBN">
												<?php 
												} 
												?>
											<input  id="excel" name="excel" type="button" class="btn btn-default" style="margin-left:10px;" value="Excel">
											<input  id="pdf" name="pdf" type="button" class="btn btn-default" style="margin-left:10px;" value="PDF">
											<input  id="print" name="print" type="button" class="btn btn-default" style="margin-left:10px;" value="Print">
									</div>
                                    ...
                                    <table id="producttable" class="table table-striped table-bordered second" style="width:100%">
                                        ...
                                        <?php
                                     		...
                                     		if (isset($_REQUEST['filter']))
											{
												...
												{
													$query = "select * from fbn_product where platform='".$platform."'";
												}
												if ($platform !="" and ($category!="" and $subcat!="") and $brand == "")
												{
													$query="SELECT * from fbn_product where platform='".$platform."' 
												...
												if ($platform !="" and ($category!="" and $subcat!="") and $brand != "")
												{
													$query="SELECT * from fbn_product where
													platform='".$platform."' 
													and category= '".$category."' and sub_category = '".$subcat."' 
													and brand='".$brand."'";
												}
												...
												if ($platform=="" and ($category=="" and $subcat=="") and $brand == "")
												{
													$query = "SELECT * FROM fbn_product";
												}
												
											}
											else if (isset($_REQUEST['searchorder']))
													{
													$query = "select * from fbn_product like '%".$_REQUEST['searchorder']."%' 
											...
											if ($numrows>0)	
											{
												...
												?>		
<tr><td><?php echo ++$i;?></td>
<td><?php $images=explode(",",$row['product_images']); ?><div class='m-r-10'><img src='/fbnproductimages/<?php echo $images[0];?>' alt='productimage' class='' width='80' height='80'></div></td><td><?php echo $row['nsi_number'];?></td><td><?php for ($l=0;$l<sizeof($brandlist);$l++){if ($brandlist[$l]['code']==$row['brand']){echo $brandlist[$l]['brand'];}}?></td><td><?php echo $row['barcode'];?></td><td><?php echo $platformname; ?></td><td><?php echo $row['product_name'] ;?></td><td><?php if ($row['stocks_quantity'] != ''){echo $row['stocks_quantity'];}else{echo 0;}?></td><td><?php echo $fifoprice; ?></td><td><?php echo $row['shelf_number'];?></td><td><a href='ecommerce-fbn-product-single.php?s_no=<?php echo $row['s_no'];?>' class='btn btn-rounded btn-primary btn-sm float-right'>View Details</a></td><?php if ($_SESSION['role']=='Superuser' or $_SESSION['role']=='Admin') {?><td><a href='edit-fbn-product.php?s_no=<?php echo $row['s_no'];?>' class='btn btn-rounded btn-success btn-sm float-right'>Edit Details</a></td><?php } ?>
		...
		$(document).on("click","#excel",function(e)
			{
				...
				location.href="fbnproductexcel.php?brand="+brand1+"&category="+category1+"&subcategory="+subcategory1+"&platform="+platform1;
			});
		...
		$(document).on("click","#print",function(e)
			{
				...
				subcategory1=$('#subcategory').val();
				platform1=$('#platform').val();
				location.href="fbnproductprint.php?brand="+brand1+"&category="+category1+"&subcategory="+subcategory1+"&platform="+platform1;
			});
			...
			$(document).on("click","#pdf",function(e)
			{
				...
				subcategory1=$('#subcategory').val();
				platform1=$('#platform').val();
				location.href="fbnproductpdf.php?brand="+brand1+"&category="+category1+"&subcategory="+subcategory1+"&platform="+platform1;
			});
			...
			$(document).on("click",".deleteproduct",function(e)
			{
				...
				if (r == true) 
				{
					...
					$.ajax({
					  method: "POST",
					  url: "ajax/deletefbnproductdata.php",
					  data:{id:productid}
					})
					  .done(function(deleter) 
					  {
						alert(deleter);
						location.href="fbntocks.php";
					});
				} 
				else 
				{
