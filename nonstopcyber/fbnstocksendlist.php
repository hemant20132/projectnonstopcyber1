    ...
    <div class="dashboard-main-wrapper">
        ...
        <div class="dashboard-wrapper">
            <div class="container-fluid dashboard-content">
                ...
                <div class="row">
                    <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                        <div class="page-header">
                            <h2 class="pageheader-title">FBN Stock Send</h2>
                            <p class="pageheader-text">FBN Stock Send List </p>
                            <div class="page-breadcrumb">
                                <nav aria-label="breadcrumb">
                                    <ol class="breadcrumb">
                                        <li class="breadcrumb-item"><a href="#" class="breadcrumb-link">Dashboard</a></li>
                                        <li class="breadcrumb-item"><a href="#" class="breadcrumb-link">FBN - Stock Send</a></li>
                ...
                <!-- ============================================================== -->
							<?php
								$platform="select * from platform  where platformname like '%FBN%' order by id";
								...
								<div class="row">
									...
									<div class="row" style="margin-top:20px;">
										...
										if ($_SESSION['role']=='Superuser' or $_SESSION['role']=='Admin') 
											{
										?>
											<input  id="stocksendfbn" name="stocksendfbn" style="margin-left:15px;" type="button" class="btn btn-default" value="FBN Stock Transfer">
										<?php 
											} 
										?>
											...
											<input  id="dailyfbnstocksend" name="dailyfbnstocksend" style="margin-left:10px;" type="button" class="btn btn-default" value="dailyfbnstocksend">
									</div>
                                        ...
                                        <?php
											...
											if (isset($_REQUEST['filter']))
											{
												...
												else
												{
													...
													if ($_SESSION['role']=='Admin' OR $_SESSION['role']=='Superuser')
													{
														...
														{
															$query = "SELECT * FROM fbnstocksend where platform='".$platform."'";
														}
														if ($fromdate!="" and $todate=="" and $platform != "")
														{
															$query = "SELECT * FROM fbnstocksend where tdate >= '".$fromdate."' and platform='".$platform."'";
														}
														if ($fromdate=="" and $todate!="" and $platform != "")
														{
															$query = "SELECT * FROM fbnstocksend where tdate <= '".$todate."' and platform='".$platform."'";
														}
														if ($fromdate!="" and $todate!="" and $platform != "")
														{
															$query = "SELECT * FROM fbnstocksend where tdate>= '".$fromdate."' and tdate <= '".$todate."' and platform='".$platform."'";
														}
													}
											...
											else
											{
												if (isset($_REQUEST['searchorder']) and ($_SESSION['role']=='Admin' or $_SESSION['role']=='Superuser'))
													{
														$query = "SELECT * FROM fbnstocksend where orderno='".$_REQUEST['searchorder']."'";
													}
												...
												if (!isset($_REQUEST['searchorder']) and ($_SESSION['role']=='Admin' or $_SESSION['role']=='Superuser'))
													{
														$query = "SELECT * FROM fbnstocksend order by id";
													}
												if (!isset($_REQUEST['searchorder']) and ($_SESSION['role']=='User'))
													{	
														$query = "SELECT * FROM fbnstocksend order by id";
												...
												if (mysqli_num_rows($result) > 0) 
												{
														while($row = mysqli_fetch_array($result,MYSQLI_ASSOC))
														{
															...
															for ($j=0;$j<sizeof($platforms);$j++)
															{
																...
																{
																	$platformname=$platforms[$j]["platformsub"];
																}	
															}	
													?>
<tr><td><?php echo $i; ?></td><td><div class="m-r-10"><img src="fbnstocksendimages/<?php echo $row['sale_image'];?>" alt="saleimage" class="rounded" height="60" width="45"></div></td><td><?php echo $row['receiptno'];?></td><td><?php echo date('d-m-Y',strtotime($row['tdate']));?></td><td><?php echo $platformname;?></td><td><?php echo $row['addtinfo']; ?><td><a href="fbn-stock-send-details-single.php?saleid=<?php echo $row['id'];?>" class='btn btn-rounded btn-primary btn-sm float-right'>View Details</a></td><?php if ($_SESSION['role']=='Superuser' or $_SESSION['role']=='Admin' or ($_SESSION['role']=='User' AND $_SESSION['username']==$row['username'])){ ?><td><a href="edit-fbnstock-send.php?saleid=<?php echo $row['id'];?>" class='btn btn-rounded btn-success btn-sm float-right'>Edit Details</a></td><?php } ?><?php if ($_SESSION['role']=='Superuser' OR $_SESSION['role']=='Admin' OR ($_SESSION['role'] == 'User' AND $_SESSION['username'] == $row['username'])){ ?><td><a href="" data-id="<?php echo $row['id']; ?>" class="btn btn-rounded btn-danger btn-sm float-right deletesale">Delete Details</a></td><?php } ?></tr>	
													<?php
														}
													   // Free result set
	...
	<script> 
		...
		{ 
			...
			$(document).on("click","#stocksendfbn", function(e)
			{
				location.href="sendstockfbn.php";
			});
	
			$(document).on("click","#dailyfbnstocksend", function(e)
			{
				location.href="daily-fbn-stock-send.php";
			});
		
			$(document).on("click","#excel", function(e)
			{
				location.href='fbnsendstockexcelreport.php?platform='+$("#platform").val()+'&fromdate='+$("#fromdate").val()+'&todate='+$("#todate").val();
			});
			
			$(document).on("click","#pdf", function(e)
			{
				location.href='fbnsendstockpdfreport.php?platform='+$("#platform").val()+'&fromdate='+$("#fromdate").val()+'&todate='+$("#todate").val();
			});
			$(document).on("click","#print", function(e)
			{
				location.href='fbnsendstockprintreport.php?platform='+$("#platform").val()+'&fromdate='+$("#fromdate").val()+'&todate='+$("#todate").val();
			});
		...
		$(document).on("click",".deletesale", function(e)
			{
				...
				if (r == true) 
				{
					...
					$.ajax({
					  method: "POST",
					  url: "ajax/deletefbnstocksend.php",
					  data:{id:saleid}
					})
