...
$subtotal=$_REQUEST['subtotal'];
	if ($_FILES['saleimg']['name'][0]!="") 
			{
					...
					foreach ($multiplefile as $name => $value) 
					{
						...
						$count++;	
						$target_dir = '../fbnsaleimages/'.$newFile; 	
	...
	if (mysqli_query($conn,$saleupdate))
	{
			...
			for ($i=0;$i<sizeof($productname);$i++)
			{
				...
				{
					$prevstock="select * from fbnsale_products where id=".$id[$i];
					$stockresult=mysqli_query($conn,$prevstock);
					$stockrow=mysqli_fetch_assoc($stockresult);
					$previousstock=$stockrow['quantity'];
					$querypr="update fbnsale_products 
					...
					if (mysqli_query($conn,$querypr))
					{
							if ($quantity[$i] > $previousstock)
							{
								...
								$updateproduct="update fbn_product set stocks_quantity=(stocks_quantity-".floatval($quantitydiff).") where  platform='".$platform."' and nsi_number='".$nsinumber[$i]."'";	
				...
				{
					$saleprinsert="INSERT INTO fbnsale_products(saleid,saleorderno, barcode, nsinumber, productname, quantity,rate,subtotal) 
					VALUES ('".$saleid."','".$orderno."','".$barcode[$i]."','".$nsinumber[$i]."','".$productname[$i]."','".$quantity[$i]."','".$rate[$i]."','".$subtotal[$i]."')";		
					mysqli_query($conn,$saleprinsert);
							//stocks update 
					$getstocks="select nsi_number,stocks_quantity from fbnproduct where nsi_number='".$nsinumber[$i]."'";
					...
					$updatestocks="update fbn_product set stocks_quantity=(stocks_quantity-".floatval($quantitydiff).") where  platform='".$platform."' and nsi_number='".$nsinumber[$i]."'";
			...
			{
			echo json_encode("error updating fbn productdata");
			}
