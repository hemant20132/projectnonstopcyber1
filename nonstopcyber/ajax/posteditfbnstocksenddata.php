	...
	if ($_FILES['saleimg']['name'][0]!="") 
			{
				...
				$saleupdate="UPDATE fbnstocksend 
	...
	if (mysqli_query($conn,$saleupdate))
	{
			...
			for ($i=0;$i<sizeof($productname);$i++)
			{
				...
				if (array_key_exists($i, $id))
				{
									...
									$prevstock="select * from fbnstocksend_products where id=".$id[$i];
									$stockresult=mysqli_query($conn,$prevstock);
									$stockrow=mysqli_fetch_assoc($stockresult);
									$previousstock=$stockrow['quantity'];
									$querypr="update fbnstocksend_products 
							...
							if ($resultstock=mysqli_query($conn,$getstocks))
							{
				...
				else
				{
							...
							// insert new data in fbnstocksend_products
							$saleprinsert="INSERT INTO fbnstocksend_products(saleid,receiptno, barcode, nsinumber, productname, quantity,fiforate) 
							...
							$resultstock=mysqli_query($conn,$getstocks);
								...
								// update product table
									...
									$updateproduct="update product set stocks_quantity=(stocks_quantity-".floatval($quantitydiff).")
									where nsi_number='".$nsinumber[$i]."'";	
									mysqli_query($conn,$updateproduct);
							//stocks update 
							$insertcount++;
				}	
			...
			{
			echo json_encode("error updating fbn stocksend productdata");
			}
