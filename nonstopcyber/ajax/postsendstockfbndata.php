	...
	if (mysqli_query($conn,$saleinsert))
	{
			...
			for ($i=0;$i<sizeof($productname);$i++)
			{
				//calculation of fifo price
					...
					$salesprinsert="INSERT INTO fbnstocksend_products(saleid,receiptno, barcode, nsinumber, productname, quantity, fiforate) 
					...
					{	
						...
						//stocks update
						//stocks update fbn product 
						$fbnstockset="update fbn_product set stocks_quantity=stocks_quantity+".floatval($quantity[$i])." where nsi_number='".$nsinumber[$i]."'" ;
						mysqli_query($conn,$fbnstockset);
						//stocks update fba product
					}
					else
					{		// if entry is not found create product entry
							$fbninsert="INSERT INTO fbn_product (s_no,category,sub_category,brand,nsi_number,barcode,product_name,product_images,size,
							...
							if (mysqli_query($conn,$fbninsert))
							{
								// copy images
										...
										{	
											...
											$newname="/home/639380.cloudwaysapps.com/zzezcecvhx/public_html/fbnproductimages/".$productimgs[$x];
								...
								$fbnstockinitial="update fbn_product set platform='".$platform."', stocks_quantity='".$quantity[$i]."' where nsi_number='".$nsinumber[$i]."'" ;
