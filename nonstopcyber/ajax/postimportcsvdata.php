            ...
            if (move_uploaded_file($_FILES["importfile"]["tmp_name"], $target_file)) 
            {
                ...
                if ($row=SimpleXLSX::parse($inputFileName)) 
                {
					...
					$productimages1=array();
							$productimage2=$rows[$i][10];
							if ($productimage2 != "")
							{
									$productimg = explode(",",$productimage1);
							}	
							$count=0;
							for ($s=0;$s<sizeof($productimg);$s++)
							{
								$fileExtn=pathinfo($productimg[$s], PATHINFO_EXTENSION);
								print_r($fileExtn);
								if ($count==0)
								{	
									$newFile = 	$productnsinumber.'.'.$fileExtn;
								}
								else
								{	
									$newFile = 	$nsinumber.'('.$count.')'.'.'.$fileExtn;
								}
								array_push($productimages,$newFile);
								$url=$productimg[$s];
								$img = '../productimages/'.$newFile;
								$content=file_get_contents($url);
								file_put_contents($img,$content);
								$count++;
								//$dir = "productimages/";
								//copy($url, "productimages/".$newFile);
							}
