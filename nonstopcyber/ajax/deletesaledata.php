...
$id=$_REQUEST['id'];
	$count=0;
		$querygetpr="select * from sale_products where saleid='".$id."'";
		...
		while ($rowpr=mysqli_fetch_array($queryrespr,MYSQLI_ASSOC))
		{
			...
			while($row=mysqli_fetch_array($queryprresult,MYSQLI_ASSOC))
			{
				...
				$saleproductdelete="Delete from sale_products where saleid='".$id."'";
				mysqli_query($conn,$saleproductdelete);
			}
			$count++;
		}

			$saledelete="delete from sale where id=".$id;
