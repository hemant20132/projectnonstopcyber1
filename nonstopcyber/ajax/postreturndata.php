...
$subtotal=$_REQUEST['subtotal'];
			if (!empty($_FILES['returnimg']['name'][0])) 
			{
		...
		$returninsert="INSERT INTO return1(return_image, orderno, platform, product_value, commissioncharges, totalamount, addtinfo, username, tdate) 
	...
	if (mysqli_query($conn,$returninsert))
	{
			...
			{
			$returnprinsert="INSERT INTO return_products(returnid,returnorderno, barcode, nsinumber, productname, quantity, isdamaged, rate,subtotal) 
			...
			{	
					...
					//stocks update
					$notetext="Added New Return for product ".$productname[$i]." - Quantity-" .$quantity[$i]." - By: ".$_SESSION['name'];
					//add notification
			...
			{
			echo json_encode("error posting return productdata");
			}
