...
$id=$_REQUEST['id'];
		$count=0;
		$querygetpr="select * from purchase_products where purchaseid='".$id."'";
		...
		while ($rowpr=mysqli_fetch_array($queryrespr,MYSQLI_ASSOC))
		{
			...
			while($row=mysqli_fetch_array($queryprresult,MYSQLI_ASSOC))
			{
				...
				$purchaseproductdelete="Delete from purchase_products where purchaseid='".$id."'";
		...
		if ($count=$numrows)
		{
			$purchasedelete="delete from purchase where id=".$id;
		}	

			if (mysqli_query($conn,$purchasedelete))
			{
