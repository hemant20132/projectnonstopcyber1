...
$returnfiles=$_FILES['returnimg'];
	if ($returnfiles['name'][0] != "")
			{
				...
				$returnupdate="UPDATE fbnreturn1 
	...
	if (mysqli_query($conn,$returnupdate))
	{
			...
			for ($i=0;$i<sizeof($productname);$i++)
			{
				...
				{
					$prevstock="select * from fbnreturn_products where id=".$id[$i];
					$stockresult=mysqli_query($conn,$prevstock);
					$stockrow=mysqli_fetch_assoc($stockresult);
					$previousstock=$stockrow['quantity'];
					$querypr="update fbnreturn_products 
				...
				//stocks update 
					if (mysqli_query($conn,$querypr))
					{
							if ($quantity[$i] > $previousstock)
							{
								...
								$updateproduct="update fbn_product set stocks_quantity=(stocks_quantity+".floatval($quantitydiff).") where nsi_number='".$nsinumber[$i]."'";	
			...
			{
			echo json_encode("error updating fbn return productdata");
			}
