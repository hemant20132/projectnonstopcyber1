...
$subtotal=$_REQUEST['subtotal'];
		...
		$saleinsert="INSERT INTO fbnsale(sale_image, orderno, platform, product_value, commissioncharges, shippingcharges, totalamount, addtinfo, username, tdate) 
	...
	if (mysqli_query($conn,$saleinsert))
	{
			...
			{
				$salesprinsert="INSERT INTO fbnsale_products(saleid,saleorderno, barcode, nsinumber, productname, quantity, rate, subtotal) 
				VALUES ('".$lastid."','".$orderno."','".$barcode[$i]."','".$nsinumber[$i]."','".$productname[$i]."','".$quantity[$i]."','".$rate[$i]."','".$subtotal[$i]."')";		
				mysqli_query($conn,$salesprinsert);
				//stocks update 
				$getstocks="select nsi_number,stocks_quantity from fbn_product where platform='".$platform."' and nsi_number='".$nsinumber[$i]."'";
				...
				$updatestocks="update fbn_product set stocks_quantity='".$replacestock."' where platform='".$platform."' and nsi_number='".$nsinumber[$i]."'";
				mysqli_query($conn,$updatestocks);
				//stocks update
				$notetext="Added New FBN Sales for product ".$productname[$i]." - Quantity-" .$quantity[$i]." - By: ".$_SESSION['name'];
				//add notification
			...
			{
			echo json_encode("error posting FBN productdata");
			}
