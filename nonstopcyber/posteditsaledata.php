...
$rate=explode(",",$_REQUEST['rate']);
	...
	if (mysqli_query($conn,$saleupdate))
	{
			...
			$insertcount++;
				...
				{
					$saleprinsert="INSERT INTO sale_products(saleid,saleorderno, barcode, nsinumber, productname, quantity,rate) 
					VALUES ('".$saleid."','".$invoiceno."','".$barcode[$i]."','".$nsinumber[$i]."','".$productname[$i]."','".$quantity[$i]."','".$rate[$i]."')";		
