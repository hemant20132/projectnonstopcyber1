...
header("Content-Disposition: attachment; filename=dailyfbnstocksend.xls");  //File name extension was wrong
header("Expires: 0");
...
$sales="select * from fbnstocksend where DATE(tdate)='".$today1."' order by id";
...
$salereport="<center><p style='font-size:15pt;'>NonStopCyber -- Daily FBN Stock Send Report- ".$today."</p><br/></center>";
...
for ($i=0;$i<sizeof($salearr);$i++)
{
			...
			for ($j=0;$j<sizeof($saleprarr);$j++)
			{
				...
				{
					$salereport.="<tr><td colspan='6'></td><td>".$platformname."</td><td>".$saleprarr[$j]['barcode']."</td><td>".$saleprarr[$j]['nsinumber']."</td><td>".$saleprarr[$j]['productname']."</td><td>".$saleprarr[$j]['quantity']."</td></tr>";
				...
				if ($firstrow==0 and ($salearr[$i]['id']==$saleprarr[$j]['saleid']))
				{
					...
					$salereport.="<td>".$saleprarr[$j]['barcode']."</td><td>".$saleprarr[$j]['nsinumber']."</td><td>".$saleprarr[$j]['productname']."</td><td>".$saleprarr[$j]['quantity']."</td><td>".$fifoprice."</td></tr>";
					$totalqty=$totalqty+$saleprarr[$j]['quantity'];	    
					$firstrow=1;
				}
			}
