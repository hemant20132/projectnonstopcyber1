                ...
                <div class="row">
                    <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                        <div class="page-header">
                            <h2 class="pageheader-title">FBN - Returns</h2>
                            <p class="pageheader-text">FBN - Returns List </p>
                            <div class="page-breadcrumb">
                                <nav aria-label="breadcrumb">
                                    <ol class="breadcrumb">
                                        <li class="breadcrumb-item"><a href="#" class="breadcrumb-link">Dashboard</a></li>
                                        <li class="breadcrumb-item"><a href="#" class="breadcrumb-link">FBN - Returns</a></li>
								...
								<div class="row">
									...
									<form id="filterform" name="filterform" method="post">
									  ...
									  <input type="date" class="form-control" id="fromdate" name="fromdate" placeholder="From Date" >
										...
										?>
										  <input type="date" value="<?php echo $todate; ?>" class="form-control" id="todate" name="todate" placeholder="To Date" >
										<?php
											}
										else
											{	
                                			...
                                			if (isset($_REQUEST['filter']))
											{
												...
												$platform=$_REQUEST['platform'];
													if ($_SESSION['role']=='Admin' OR $_SESSION['role']=='Superuser')
													{
														...
														{
						$query = "SELECT a.*, b.* FROM fbnreturn1 as a, fbnreturn_products as b where a.id=a.returnid and a.platform='".$platform."' order by a.id";
														}
														if ($fromdate!="" and $todate=="" and $platform != "")
														{
						$query = "SELECT a.*, b.* FROM fbnreturn1 as a, fbnreturn_products as b where a.tdate >= '".$fromdate."' order by a.id";
														}
														if ($fromdate=="" and $todate!="" and $platform != "")
														{
						$query = "SELECT a.*, b.* FROM fbnreturn1 as a, fbnreturn_products as b where a.id=b.returnid and a.tdate <= '".$todate."' and a.platform='".$platform."' order by a.id";
														}
														if ($fromdate!="" and $todate!="" and $platform != "")
														{
						$query = "SELECT a.*, b.* FROM fbnreturn1 as a, fbnreturn_products as b where a.id=b.returnid and a.tdate>= '".$fromdate."' and a.tdate <= '".$todate."' and a.platform='".$platform."' order by a.id";
														}	
													}
													...
													if ($_SESSION['role']=='User')
													{
														if ($fromdate=="" and $todate=="" and $platform != "")
														{
						$query = "SELECT a.*,b.* FROM fbnreturn1 as a, fbnreturn_products as b where a.id=b.returnid and a.platform='".$platform."' order by a.id";
														}
														if ($fromdate!="" and $todate=="" and $platform != "")
														{
						$query = "SELECT a.*,b.* FROM fbnreturn1 as a, fbnreturn_products as b where a.id=b.returnid and a.tdate >= '".$fromdate."' and a.platform='".$platform."' order by a.id";
														}
														if ($fromdate=="" and $todate!="" and $platform != "")
														{
						$query = "SELECT a.*, b.* FROM fbnreturn1 as a, fbnreturn_products as b where a.id=b.returnid and a.tdate <= '".$todate."' and a.platform='".$platform."' order by a.id";
						...
						$query = "SELECT a.*,b.* FROM fbnreturn1 as a, fbnreturn_products as b where a.id=b.returnid and a.tdate>= '".$fromdate."' and a.tdate <= '".$todate."' and a.platform='".$platform."' order by a.id";
											...
											else
											{
												if (isset($_REQUEST['searchorder']) and ($_SESSION['role']=='Admin' or $_SESSION['role']=='Superuser'))
													{
						$query = "SELECT a.*,b.* FROM fbnreturn1 as a, fbnreturn_products as b where a.id=b.returnid and a.orderno='".$_REQUEST['searchorder']."' order by a.id";
													}
												if (isset($_REQUEST['searchorder']) and ($_SESSION['role']=='User'))
													{	
						$query = "SELECT a.*,b.* FROM fbnreturn1 as a, fbnreturn_products as b where a.id=b.returnid and a.orderno='".$_REQUEST['searchorder']."' and a.username='".$_SESSION['username']."' order by a.id";
													}
												if (!isset($_REQUEST['searchorder']) and ($_SESSION['role']=='Admin' or $_SESSION['role']=='Superuser'))
												{
						$query = "SELECT a.*,b.* FROM fbnreturn1 as a, fbnreturn_products as b where a.id=b.returnid order by a.id";
												}		
												if (!isset($_REQUEST['searchorder']) and ($_SESSION['role']=='User'))
												{
						$query = "SELECT a.*, b.* FROM fbnreturn1 as a, fbnreturn_products as b where a.id=b.returnid";
												}	
											}
											$i = 0;
...
<tr><td><?php echo $i; ?></td><td><div class="m-r-10"><img src="fbnreturnimages/<?php echo $images[0]; ?>" alt="user" class="rounded" height="60" width="45"></div></td><td><?php echo $row['orderno'];?></td><td><?php echo $platformname;?></td><td><?php echo $row['totalamount'];?></td><td><?php echo $row['addtinfo']; ?></td><td><?php echo $row['isdamaged'] ; ?></td><td><a href="fbn-return-details-single.php?returnid=<?php echo $row['returnid'];?>" class='btn btn-rounded btn-primary btn-sm  float-right'>View Details</a></td><?php if ($_SESSION['role']=='Superuser' OR $_SESSION['role']=='Admin' OR ($_SESSION['role'] == 'User' AND $_SESSION['username'] == $row['username'])){ ?><td><a href="edit-fbn-return.php?returnid=<?php echo $row['returnid'];?>" class='btn btn-rounded btn-success btn-sm float-right'>Edit Details</a></td><td><a href="" data-id="<?php echo $row['returnid'];?>" class='btn btn-rounded btn-danger btn-sm float-right deletereturn'>Delete Details</a></td><?php } ?></tr>	
													<?php
	...
	<script> 
		...
		{ 
			...
			{	
				...
				buttons: 
				[
				{text:'Add FBN return',className:'btn btn-primary addreturn',
						action: function ( e, dt, node, config ) 
						{ location.href='fbnreturn.php'
						}
				},
				'copy',
				{
                    ...
                    exportOptions: 
					{
						...
						action: function ( e, dt, node, config ) 
						{ location.href='returnfbnexcelreport.php'
						}
				},
				{
                    extend: 'pdf',
